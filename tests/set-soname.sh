...
if ../src/patchelfmod -d --print-soname ${SCRATCH}/libbar-withsoname.so = libbar.so.0; then
...
if ../src/patchelfmod -d --print-soname ${SCRATCH}/libbar-withsoname.so = libtest.so.0; then
